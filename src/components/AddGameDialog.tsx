...
export function AddGameDialog({ onAddGame, trigger }: AddGameDialogProps) {
    ...
    const handleAdd = async (game: EnrichedGame) => {
        if (addingGameId) return; // Prevent duplicate adds

        setAddingGameId(game.id);

        // ⚡ OPTIMISTIC UPDATE: Close dialog immediately for better UX
        ...
        // Create minimal game object with existing data
        const minimumGame = {
            id: game.id.toString(),
            title: game.name,
